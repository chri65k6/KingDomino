...
TERRAIN_DIR = BASE_DIR / "TerrainClassification"
DATASET_DIR = BASE_DIR / "King Domino dataset"
DEFAULT_PARAMS_PKL = BASE_DIR / "kingdomino_params.pkl"
DEFAULT_MODEL_PKL = TERRAIN_DIR / "king_domino_final_model.pkl"
DEFAULT_TEMPLATES_DIR = BASE_DIR / "Templates"
DEFAULT_IMAGE = DATASET_DIR / "73.jpg"

TERRAIN_CLASSES = {"field", "forest", "grass", "mine", "swamp", "water"}
CASTLE_CLASSES = {"castle", "home"}
INVALID_HARMONY_CLASSES = {"", "x", "unknown", "empty", "none"}
MIDDLE_KINGDOM_BONUS = 10
HARMONY_BONUS = 5
CLUSTER_COLORS = [
    (64, 180, 255),
    (86, 204, 113),
...
if TERRAIN_DIR.is_dir() and str(TERRAIN_DIR) not in sys.path:
    sys.path.insert(0, str(TERRAIN_DIR))

try:
    from kingdomino_feature_pipeline import (
        create_sift as pipeline_create_sift,
        descriptors_to_sift_histogram as pipeline_descriptors_to_sift_histogram,
        extract_hog_feature_array as pipeline_extract_hog_feature_array,
        extract_hsv_feature_values as pipeline_extract_hsv_feature_values,
        extract_sift_descriptors as pipeline_extract_sift_descriptors,
    )

    USING_TERRAIN_PIPELINE = True
except Exception:
    USING_TERRAIN_PIPELINE = False
...
def resolve_path(path):
    ...
    if not path.suffix:
        dataset_jpg_path = DATASET_DIR / f"{path}.jpg"
        if dataset_jpg_path.is_file():
            return dataset_jpg_path

    project_path = BASE_DIR / path
    if project_path.is_file():
        return project_path

    terrain_path = TERRAIN_DIR / path
...
def load_crown_templates(params):
    ...
    params_template_dir = Path(params.get("TEMPLATE_DIR", DEFAULT_TEMPLATES_DIR))

    if not template_dir.is_dir() and params_template_dir.is_dir():
        template_dir = params_template_dir
...
def extract_hsv_features(tile):
    if USING_TERRAIN_PIPELINE:
        values = pipeline_extract_hsv_feature_values(tile)
        return {
            "hue": float(values["hue"]),
...
def extract_sift_features(tile, kmeans, feature_params, sift_detector=None):
    if USING_TERRAIN_PIPELINE:
        sift = sift_detector or pipeline_create_sift()
        descriptors, num_keypoints = pipeline_extract_sift_descriptors(tile, sift)
        histogram = pipeline_descriptors_to_sift_histogram(descriptors, kmeans)
        features = {"num_sift_keypoints": float(num_keypoints)}
...
def extract_hog_features(tile, feature_params, expected_length):
    if USING_TERRAIN_PIPELINE:
        features = np.asarray(pipeline_extract_hog_feature_array(tile), dtype=float)
...
def extract_tile_features(tile, row, col, image_path, bundle, sift_detector=None):
    ...
    features.update(extract_sift_features(tile, bundle["sift_kmeans"], feature_params, sift_detector))
...
def classify_terrains(image, image_path, bundle):
    ...
    sift_detector = pipeline_create_sift() if USING_TERRAIN_PIPELINE else None

    rows = []
    for row in range(board_size):
        for col in range(board_size):
            rows.append(extract_tile_features(tiles[row][col], row, col, image_path, bundle, sift_detector))
...
def has_complete_board_for_harmony(terrain_grid):
    board_size = len(terrain_grid)
    ...
    for row in terrain_grid:
        ...
        for terrain in row:
            if str(terrain).strip().lower() in INVALID_HARMONY_CLASSES:
                return False
...
def calculate_bonuses(
    terrain_grid,
    use_middle_kingdom_bonus=True,
    use_harmony_bonus=True,
):
    bonuses = []
    board_size = len(terrain_grid)

    if use_middle_kingdom_bonus and board_size % 2 == 1:
        center = board_size // 2
        center_terrain = terrain_grid[center][center]

        if center_terrain in CASTLE_CLASSES:
            bonuses.append(
                {
                    "name": "Middle Kingdom",
                    "points": MIDDLE_KINGDOM_BONUS,
                    "description": "castle/home is placed in the center tile",
                    "cell": [center, center],
                }
            )

    if use_harmony_bonus and has_complete_board_for_harmony(terrain_grid):
        bonuses.append(
            {
                "name": "Harmony",
                "points": HARMONY_BONUS,
                "description": "all board positions are filled",
            }
        )
...
def print_result(
    ...
    if bonuses:
        ...
        for bonus in bonuses:
            print(f"  {bonus['name']}: +{bonus['points']} points")
...
def result_to_dict(
    ...
    return {
        ...
        "bonus_points": sum(bonus["points"] for bonus in bonuses or []),
        "bonuses": bonuses or [],
...
def draw_rounded_rect(image, top_left, bottom_right, color, radius=8, alpha=1.0):
    overlay = image.copy()
    x1, y1 = top_left
    x2, y2 = bottom_right
...
def draw_text_with_background(
    ...
    draw_rounded_rect(
        image,
        (max(0, x - pad_x), max(0, y - height - pad_y)),
        (min(image.shape[1] - 1, x + width + pad_x), min(image.shape[0] - 1, y + baseline + pad_y)),
        bg_color,
        radius=radius,
        alpha=alpha,
    )
...
def put_panel_text(image, text, origin, font_scale, color, thickness=1):
    cv.putText(image, text, origin, cv.FONT_HERSHEY_SIMPLEX, font_scale, color, thickness, cv.LINE_AA)
...
def draw_wrapped_lines(image, lines, x, start_y, max_y, line_height=24):
    ...
    for line in lines:
        if y > max_y:
            put_panel_text(image, "...", (x, y), 0.55, (90, 90, 90), 1)
            break
        put_panel_text(image, line, (x, y), 0.48, (55, 55, 55), 1)
        y += line_height
...
def draw_cluster_boundaries(canvas, component, color, tile_width, tile_height):
    ...
    shadow_color = (24, 28, 32)
    for start, end in edges:
        cv.line(canvas, start, end, shadow_color, 7, cv.LINE_AA)

    for start, end in edges:
        cv.line(canvas, start, end, color, 4, cv.LINE_AA)


def choose_score_badge_cell(component, crown_grid):
    cells = [(row, col) for row, col in component["cells"]]
    cell_set = set(cells)
    ...
    def score_cell(cell):
        ...
        crowns = int(crown_grid[row][col])
        return (same_neighbors * 12) - (distance * 3) - (crowns * 2)

    return max(cells, key=score_cell)


def draw_score_badge(board, component, crown_grid, tile_width, tile_height):
    row, col = choose_score_badge_cell(component, crown_grid)
    label = f"{component['points']}p"
    font_scale = 0.48
    thickness = 2
    ...
    x1 = col * tile_width
    y1 = row * tile_height
    badge_x = x1 + tile_width - label_width - 14
    badge_y = y1 + 54
...
def render_score_image(
    ...
    board = cv.addWeighted(overlay, 0.16, board, 0.84, 0)

    for row in range(board_size + 1):
        y = row * tile_height
        cv.line(board, (0, y), (board.shape[1], y), (255, 255, 255), 1)

    for col in range(board_size + 1):
        x = col * tile_width
        cv.line(board, (x, 0), (x, board.shape[0]), (255, 255, 255), 1)
    ...
    for row in range(board_size):
        for col in range(board_size):
            ...
            crown_bg = (31, 132, 198) if crowns else (36, 40, 44)
            draw_text_with_background(
                board,
                f"K: {crowns}",
                (x + 7, y + tile_height - 11),
                0.48,
                (255, 255, 255),
                bg_color=crown_bg,
                alpha=0.80,
    ...
    for index, component in enumerate(components, start=1):
        ...
        draw_score_badge(board, component, crown_grid, tile_width, tile_height)

    panel_width = 440
    panel = np.full((board.shape[0], panel_width, 3), (247, 246, 242), dtype=np.uint8)
    cv.line(panel, (0, 0), (0, panel.shape[0]), (218, 216, 208), 2)

    put_panel_text(panel, "Kingdomino", (24, 39), 0.92, (34, 34, 34), 2)
    put_panel_text(panel, "Pointberegner", (26, 66), 0.46, (110, 105, 96), 1)

    draw_rounded_rect(panel, (22, 86), (panel_width - 22, 158), (231, 246, 236), radius=14, alpha=1.0)
    put_panel_text(panel, "Samlet score", (42, 114), 0.50, (68, 92, 73), 1)
    put_panel_text(panel, str(total_points), (42, 146), 1.02, (23, 111, 54), 2)

    if bonuses:
        bonus_points = sum(bonus["points"] for bonus in bonuses)
        detail = f"Terrain {terrain_points}  +  bonus {bonus_points}"
        (detail_width, _), _ = cv.getTextSize(detail, cv.FONT_HERSHEY_SIMPLEX, 0.42, 1)
        put_panel_text(panel, detail, (panel_width - 42 - detail_width, 146), 0.42, (71, 103, 78), 1)

    put_panel_text(panel, "Point pr. cluster", (24, 200), 0.62, (45, 45, 45), 2)
    put_panel_text(panel, "felter x kroner = point", (26, 224), 0.43, (115, 110, 100), 1)
    ...
    if not scoring_components:
        draw_rounded_rect(panel, (24, 244), (panel_width - 24, 292), (255, 255, 255), radius=10, alpha=1.0)
        put_panel_text(panel, "Ingen scorende clusters.", (42, 275), 0.50, (70, 70, 70), 1)
    else:
        bonus_block_height = 0
        if bonuses:
            bonus_block_height = 36 + (len(bonuses) * 38)

        cluster_max_y = panel.shape[0] - 16 - bonus_block_height
        row_y = 244
        row_height = 42
        for index, component in scoring_components:
            if row_y + row_height > cluster_max_y:
                put_panel_text(panel, "...", (32, max(270, cluster_max_y - 6)), 0.58, (100, 100, 100), 1)
                break
            ...
            draw_rounded_rect(
                panel,
                (24, row_y),
                (panel_width - 24, row_y + 34),
                (255, 255, 255),
                radius=10,
                alpha=1.0,
            )
            draw_rounded_rect(panel, (38, row_y + 9), (58, row_y + 27), color, radius=5, alpha=1.0)
            put_panel_text(
                panel,
            ...
            formula = f"{component['size']} x {component['crowns']} = {component['points']}"
            (formula_width, _), _ = cv.getTextSize(formula, cv.FONT_HERSHEY_SIMPLEX, 0.47, 1)
            put_panel_text(
                panel,
                formula,
                (panel_width - 42 - formula_width, row_y + 23),
                0.47,
                (34, 92, 53),
                1,
            )
    ...
    if bonuses:
        bonus_block_height = 36 + (len(bonuses) * 38)
        bonus_y = panel.shape[0] - bonus_block_height + 8
        put_panel_text(panel, "Bonus", (24, bonus_y), 0.58, (45, 45, 45), 2)
        bonus_y += 20
        for bonus in bonuses:
            if bonus_y + 32 > panel.shape[0] - 10:
                break
            draw_rounded_rect(panel, (24, bonus_y), (panel_width - 24, bonus_y + 32), (255, 255, 255), radius=9)
            put_panel_text(panel, bonus["name"], (42, bonus_y + 22), 0.44, (55, 55, 55), 1)
            text = f"+{bonus['points']}"
            (text_width, _), _ = cv.getTextSize(text, cv.FONT_HERSHEY_SIMPLEX, 0.48, 1)
            put_panel_text(panel, text, (panel_width - 42 - text_width, bonus_y + 22), 0.48, (23, 111, 54), 1)
            bonus_y += 38
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help=f"Path, filename, or image number for a cropped 5x5 board image. Default: {DEFAULT_IMAGE}",
    )
    ...
    parser.add_argument(
        "--no-middle-kingdom-bonus",
        action="store_true",
        help="Disable the +10 bonus for castle/home in the center tile.",
    )
    parser.add_argument(
        "--no-harmony-bonus",
        action="store_true",
        help="Disable the +5 Harmony bonus for a complete board.",
    )
...
def main():
    ...
    terrain_points, components = score_board(terrain_grid, crown_grid)
    bonuses = calculate_bonuses(
        terrain_grid,
        use_middle_kingdom_bonus=not args.no_middle_kingdom_bonus,
        use_harmony_bonus=not args.no_harmony_bonus,
    )
    total_points = terrain_points + sum(bonus["points"] for bonus in bonuses)
    ...
    visualization = render_score_image(
        image,
        terrain_grid,
        crown_grid,
        components,
